...
### Medical/research-safe workspace with citations, evidence grids, warning panels, and human-review gates.
...
**Domain:** `Education / Research` · **Suite:** `Medical & Research Suite` · **Accent:** `#5eead4`
...
**Paper Explainer** is a standalone, browser-based AI workflow app for **Education / Research**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
**UX profile:** `Evidence & Safety Workbench`

**Workflow layout:** Source intake → evidence extraction → safety review → study/learning output

**Empty state:** Paste paper text, notes, or upload source files. Clinical/medical decisions require expert verification.
...
|---|---:|:---:|---|---|
| `paper`<br>Paper | text | Yes | — | Affects input: Paper. |
| `doi`<br>DOI | text | Yes | — | Affects input: DOI. |
| `pdf`<br>PDF | text | Yes | — | Affects input: PDF. |
...
| `field`<br>field | text | No | — | Affects customization: field. |
| `audience_level`<br>audience level | select | No | general | Affects customization: audience level. |
| `output_depth`<br>output depth | text | No | — | Affects customization: output depth. |
| `citation_style`<br>citation style | select | No | APA | Affects customization: citation style. |
| `focus_section`<br>focus section | text | No | — | Affects customization: focus section. |
| `language`<br>language | select | No | English | Affects customization: language. |
| `critique_strictness`<br>critique strictness | slider | No | 50 | Affects customization: critique strictness. |
...
- **citation style**: APA, Vancouver, MLA, Chicago, None
...
uvicorn app.main:app --host 127.0.0.1 --port 9143
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9143`. |
...
- validated clinical/drug/evidence database for medical claims
- licensed clinician review before use in care
- PHI-safe storage and access controls
...
```bash
curl http://127.0.0.1:9143/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9143/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9143 uvicorn app.main:app --host 0.0.0.0 --port 9143
```
...
- `plugins/paper-explainer.json` for plugin metadata.
